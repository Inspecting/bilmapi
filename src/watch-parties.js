...
function config(env) {
  return {
    ...
    presenceWriteMs: numberSetting(env?.WATCH_PARTY_PRESENCE_WRITE_MS, 60_000, 15_000, 300_000)
  };
}
...
function storageFor(env) {
  if (env?.BILM_DB && typeof env.BILM_DB.prepare === 'function') return { type: 'd1', value: env.BILM_DB };
  if (env?.BILM_DATA && typeof env.BILM_DATA.get === 'function' && typeof env.BILM_DATA.put === 'function') {
    return { type: 'kv', value: env.BILM_DATA };
  }
  return null;
}

async function deleteParty(storage, code) {
  if (storage.type === 'd1') {
    await storage.value.prepare('DELETE FROM watch_parties WHERE code = ?1').bind(code).run();
    return;
  }
  await storage.value.delete(keyFor(code));
}

async function load(storage, code) {
  let raw = null;
  if (storage.type === 'd1') {
    const row = await storage.value.prepare('SELECT party_json FROM watch_parties WHERE code = ?1').bind(code).first();
    raw = row?.party_json || null;
  } else {
    raw = await storage.value.get(keyFor(code));
  }
  if (!raw) return null;
  try {
    const party = JSON.parse(raw);
    if (!party || Number(party.expiresAt || 0) <= Date.now()) {
      await deleteParty(storage, code);
      return null;
    }
...
async function save(storage, party, ttlMs = 0) {
  if (Number(ttlMs) > 0) {
    party.updatedAt = Date.now();
    party.expiresAt = party.updatedAt + Number(ttlMs);
  }
  if (storage.type === 'd1') {
    await storage.value.prepare(`
      INSERT INTO watch_parties (code, party_json, expires_at_ms, updated_at_ms)
      VALUES (?1, ?2, ?3, ?4)
      ON CONFLICT(code) DO UPDATE SET
        party_json = excluded.party_json,
        expires_at_ms = excluded.expires_at_ms,
        updated_at_ms = excluded.updated_at_ms
    `).bind(party.code, JSON.stringify(party), Number(party.expiresAt || 0), Number(party.updatedAt || Date.now())).run();
    return;
  }
  const seconds = Math.max(60, Math.ceil((Number(party.expiresAt || 0) - Date.now()) / 1000));
  await storage.value.put(keyFor(party.code), JSON.stringify(party), { expirationTtl: seconds });
}
...
function publicParty(party, participantId, graceMs, presenceWriteMs = 60_000) {
  const now = Date.now();
  const connectedWindowMs = Math.min(graceMs, Math.max(20_000, presenceWriteMs + 30_000));
  ...
  return {
    ...
    participants: party.participants.map((person) => ({
      ...
      isConnected: person.id === participantId || now - Number(person.lastSeen || 0) < connectedWindowMs
    })),
    hostId: party.hostId,
...
async function create({ request, env, storage }) {
  ...
  for (let attempt = 0; attempt < 12; attempt += 1) {
    ...
    if (await load(storage, code)) continue;
    ...
    await save(storage, party, limits.ttlMs);
    return response(201, { party: publicParty(party, participantId, limits.graceMs, limits.presenceWriteMs), participantToken, testingLimit: limits.maxSize }, request);
  }
  return response(503, { error: 'Unable to allocate a party code. Please try again.' }, request);
}

async function mutate({ request, env, storage, code, action }) {
  const limits = config(env);
  const party = await load(storage, code);
  ...
  if (action === 'join') {
    ...
    if (returning) {
      ...
      await save(storage, party, limits.ttlMs);
      return response(200, { party: publicParty(party, returning.id, limits.graceMs, limits.presenceWriteMs), participantToken: returning.token, reconnected: true }, request);
    }
    ...
    await save(storage, party, limits.ttlMs);
    return response(200, { party: publicParty(party, participantId, limits.graceMs, limits.presenceWriteMs), participantToken }, request);
  }
  ...
  const previousLastSeen = Number(participant.lastSeen || 0);
  participant.lastSeen = Date.now();
  prune(party, limits.graceMs);

  if (action === 'heartbeat') {
    if (participant.lastSeen - previousLastSeen >= limits.presenceWriteMs) {
      await save(storage, party, limits.ttlMs);
    }
    return response(200, { party: publicParty(party, participant.id, limits.graceMs, limits.presenceWriteMs) }, request);
  }
  if (action === 'leave') {
...
export async function handleWatchPartyRequest({ request, env }) {
  ...
  try {
    if (request.method === 'POST' && url.pathname === '/watch-parties') return await create({ request, env, storage });
    const match = url.pathname.match(/^\/watch-parties\/([A-Za-z0-9]{4,12})\/(join|state|permissions|heartbeat|leave)$/);
    if (request.method === 'POST' && match) return await mutate({ request, env, storage, code: cleanCode(match[1]), action: match[2] });
